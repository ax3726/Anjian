...
public class AddWeiHuaActivity extends PhotoActivity<BasePresenter, ActivityAddWeiHuaBinding> {
    ...
    private void submitMessage() {
        ...
        if (mUType==0) {
            ...
            Api.getApi().addWeiHua1(getRequestBody(addWeiHuaRequest), MyApplication.getInstance().getToken()).compose(callbackOnIOToMainThread()).subscribe(new BaseNetListener<BaseBean>(this, true) {
                @Override
                public void onSuccess(BaseBean baseBean) {
                    showToast(baseBean.getMessage());
                    EventBus.getDefault().post("刷新");
